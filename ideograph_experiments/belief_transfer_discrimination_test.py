...
class BeliefTransferDiscriminationConfig:
    feature_count: int = 8
...
def parse_args() -> BeliefTransferDiscriminationConfig:
    parser = argparse.ArgumentParser(
        description=(
            "Test whether a transferred synthetic schema helps on a similar conflict "
            "and hurts on a dissimilar conflict."
        )
    )
    ...
    config = BeliefTransferDiscriminationConfig(
...
def validate_config(config: BeliefTransferDiscriminationConfig) -> None:
    if config.feature_count != 8:
        raise ValueError("belief_transfer_discrimination_test currently expects feature_count == 8")
        ...
        if (
            ...
            if to_add:
                ...
                if policy_name == "scratch_synthesis":
                    spawn_episode = episode_index
                elif policy_name == "transfer_schema" and spawn_episode == 0:
                    spawn_episode = episode_index

        update_scores(scores, hypotheses, world, true_action, config.consistency)

    final_top_name, _ = top_hypothesis(scores, hypotheses)
    final_top_mass = top_hypothesis(scores, hypotheses)[1]
    return TargetResult(
...
def print_report(
    ...
    print("Experiment: positive vs negative transfer of a synthetic belief schema")
    ...
    print()
    print("Transfer discrimination summary:")
    for target_name, results in sorted(target_groups.items()):
        ...
        transfer = next(result for result in results if result.policy_name == "transfer_schema")
        delta_first_ten = transfer.first_ten_accuracy - scratch.first_ten_accuracy
        delta_overall = transfer.overall_accuracy - scratch.overall_accuracy
        print(
            f"{target_name}: "
            f"delta_first10={delta_first_ten:+.3f}, "
            f"delta_overall={delta_overall:+.3f}"
        )
