...
class BeliefMultiTransferStoppingConfig:
    feature_count: int = 8
...
class PolicyResult:
    ...
    predicted_table_value: float
    true_table_accuracy: float
...
def parse_args() -> BeliefMultiTransferStoppingConfig:
    parser = argparse.ArgumentParser(
        description=(
            "Test whether an agent can stop asking diagnostic questions early "
            "when multiple transfer schemas compete and questions have a cost."
        )
    )
    ...
    parser.add_argument("--top-mass-threshold", type=float, default=0.80)
    ...
    config = BeliefMultiTransferStoppingConfig(
...
def validate_config(config: BeliefMultiTransferStoppingConfig) -> None:
    if config.feature_count != 8:
        raise ValueError("belief_multi_transfer_stopping_test currently expects feature_count == 8")
...
def initial_scores(config: BeliefMultiTransferStoppingConfig) -> list[float]:
    return [
...
def true_table_accuracy(
    ...
    for world in eval_worlds:
        ...
        accuracies.append(1 if predicted_action == target_value(target_name, world) else 0)
    return statistics.mean(accuracies)


def best_probe_future_value(
    scores: Sequence[float],
    hypotheses: Sequence[Hypothesis],
    ...
    for probe_index, probe in enumerate(probe_bank):
        ...
        for outcome in (0, 1):
            ...
            expected_future_value += outcome_probability * predicted_table_value(
                projected_scores,
                hypotheses,
                eval_worlds,
            )

        if expected_future_value > best_value:
            best_value = expected_future_value
...
def run_policy(
    ...
    for _ in range(config.diagnostic_budget):
        probe_index, best_future_value = best_probe_future_value(
            scores,
            hypotheses,
            probe_bank,
            asked_indices,
        ...
        if policy_name == "value_stop":
            current_value = predicted_table_value(scores, hypotheses, eval_worlds)
            remaining_probes = len(probe_bank) - len(asked_indices)
            if remaining_probes == 0:
                stop_reason = "probe_bank_exhausted"
                break

            _, next_future_value = best_probe_future_value(
                scores,
                hypotheses,
                probe_bank,
                asked_indices,
    ...
    predicted_value = predicted_table_value(scores, hypotheses, eval_worlds)
    table_accuracy = true_table_accuracy(scores, hypotheses, target_name, eval_worlds)
    net_true_value = table_accuracy - config.question_cost * len(probe_plan)
    ...
    return PolicyResult(
        ...
        predicted_table_value=predicted_value,
        true_table_accuracy=table_accuracy,
        net_true_value=net_true_value,
...
def print_report(
    ...
    print("Experiment: stopping diagnostic questions under competing transfer schemas")
    ...
    for result in results:
        print(
            ...
            f"{result.predicted_table_value:>8.3f}"
            f"{result.true_table_accuracy:>8.3f}"
            f"{result.net_true_value:>8.3f}"
    ...
    for policy_name, policy_results in grouped.items():
        plans = ";".join(result.probe_plan for result in policy_results)
        print(
            f"{policy_name:<16}"
            f"{statistics.mean(result.questions_asked for result in policy_results):>8.3f}"
            f"{statistics.mean(result.true_table_accuracy for result in policy_results):>12.3f}"
            f"{statistics.mean(result.net_true_value for result in policy_results):>11.3f}"
