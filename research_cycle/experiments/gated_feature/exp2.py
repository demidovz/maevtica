#!/usr/bin/env python3
"""Gated Feature falsification, RUN 2 (Amendment 1: max-over-positions KL). PREREG.md.
...
# ---- clean per-position logits + noise floor (Amendment 1: max over positions) ----
clean_lp_all, clean_p_all = [], []
for t in toks_list:
    lp = Fn.log_softmax(m(t)[0].float(), -1)     # [seq, vocab]
    clean_lp_all.append(lp); clean_p_all.append(lp.exp())
def kl_pos(pi, abl_lp):                          # abl_lp [bs, seq, vocab] -> [bs, seq]
...
nz = Fn.log_softmax(m.run_with_hooks(toks_list[0], fwd_hooks=[(HOOK, zfn)])[0].float(), -1)
noise = float(kl_pos(0, nz.unsqueeze(0)).max())
...
for pi, (t, f) in enumerate(zip(toks_list, feats)):
    ...
    for s in range(0, live.numel(), CHUNK):
        ...
        lg = m.run_with_hooks(t.repeat(bs, 1), fwd_hooks=[(HOOK, fn)]).float()  # [bs, seq, vocab]
        kls = kl_pos(pi, Fn.log_softmax(lg, -1))              # [bs, seq]
        seq = lg.shape[1]
        first_fire = (acts > 0).float().argmax(0)             # [bs] first firing pos (f[0]=0)
        pos_idx = torch.arange(seq).unsqueeze(0)              # [1, seq]
        mask = pos_idx >= first_fire.unsqueeze(1)             # only positions at/after firing
