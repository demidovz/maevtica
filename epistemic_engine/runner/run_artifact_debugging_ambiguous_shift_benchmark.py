...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark for artifact-level ambiguous shift / false alarm environment."
    )
...
def main() -> None:
    ...
    policies = [
        ...
        LatentTrustShiftMemoryPolicy(
            ...
            neutral_gate=0.50,
            min_aggressive_gate=0.0,
            min_observation_weight=0.45,
            false_alarm_discount=0.70,
            persistence_relief=0.35,
        ),
    ]
    ...
    print(
        f"Environment: artifact_ambiguous_shift | confidence>={args.confidence_threshold}, "
        f"max_cost={args.max_cost}, max_steps={args.max_steps}, "
        f"shift_after_step={args.shift_after_step}, shift_probability={args.shift_probability}, "
        f"false_alarm_length={args.false_alarm_length}, min_stop=4"
    )
    print_table(rows)
