...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Signature report for parser step3 states after diff + scope."
    )
...
def margin_bucket(probabilities: dict[str, float]) -> str:
    ordered = sorted(probabilities.values(), reverse=True)
    top_value = ordered[0]
    second_value = ordered[1] if len(ordered) > 1 else 0.0
...
def scope_category(outcome: str) -> str:
    normalized = outcome.lower()
    if "old accounts" in normalized or "older data" in normalized or "migration rollout" in normalized:
        return "old_data"
    if "traffic spikes" in normalized or "parallel load" in normalized or "retries sometimes hide" in normalized:
        return "load"
    if "first attempt" in normalized or "warm requests" in normalized or "repeated requests" in normalized or "restart briefly helps" in normalized:
        return "warm_state"
    if "staging" in normalized or "canary" in normalized or "env pack" in normalized or "prod pods" in normalized or "fresh images" in normalized:
        return "environment"
    if "payload" in normalized or "input data" in normalized or "fixtures" in normalized or "unicode" in normalized:
        return "payload"
    return "generic"


def diff_category(outcome: str) -> str:
    normalized = outcome.lower()
    if "parser/decoder.py" in normalized or "payload sanitizer" in normalized or "payload normalization" in normalized:
        return "ingest_path"
    if "poetry.lock" in normalized or "dependency pins" in normalized or "extra requirement" in normalized:
        return "dependency"
    if "cache" in normalized or "retry" in normalized:
        return "state"
    if "migration" in normalized or "legacy_tier" in normalized or "schema" in normalized:
        return "migration"
    if "worker pool" in normalized or "inflight_jobs" in normalized or "concurrency" in normalized:
        return "runtime"
    if "staging.env" in normalized or "config loader" in normalized or "deploy wiring" in normalized:
        return "config"
    return "generic"
...
def collect_rows(args: argparse.Namespace) -> list[dict[str, object]]:
    policy = build_policy()
    score_buckets: dict[tuple[str, str, str, str], dict[str, list[float]]] = defaultdict(
        lambda: defaultdict(list)
    )
    default_action_buckets: dict[tuple[str, str, str, str], list[str]] = defaultdict(list)
    scenario_buckets: dict[tuple[str, str, str, str], list[str]] = defaultdict(list)
    hypothesis_buckets: dict[tuple[str, str, str, str], list[str]] = defaultdict(list)
    ...
    for episode_no in range(args.episodes):
        ...
        default_action_buckets[signature].append(current_action_id)
        scenario_buckets[signature].append(env.scenario_label())
        hypothesis_buckets[signature].append(env.actual_hypothesis)
        ...
        for action in candidate_actions(state, env):
            ...
            score_buckets[signature][action.action_id].append(value)
    ...
    for signature, action_scores in ranked_signatures[: args.top_k]:
        ...
        default_action_id = Counter(default_action_buckets[signature]).most_common(1)[0][0]
        scenarios = scenario_buckets[signature]
        hypotheses = Counter(hypothesis_buckets[signature]).most_common(1)[0][0]
        rows.append(
            {
                "margin": signature[0],
                "risk": signature[1],
...
def main() -> None:
    ...
    print(
        f"Parser step3 signature report | episodes={args.episodes}, "
